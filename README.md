
###  **Project Overview: Bright Coffee Shop Sales Analysis**

This project presents a comprehensive sales analysis of historical transactional data from **Bright Coffee Shop**. The objective is to generate actionable business insights to support a newly appointed CEO in improving revenue performance, identifying growth opportunities, and optimizing product strategy.

The analysis explores revenue trends, customer behavior, and product performance to enable data-driven decision-making.

---

###  **Project By**

**Refilwe Sebako**
 Pretoria, South Africa
 [refilwee.sebako@gmail.com](mailto:refilwee.sebako@gmail.com)
 LinkedIn:www.linkedin.com/in/resebako

 **Aspiring Data Analyst | Virtual Assistant (10+ years experience)**
Skilled in data analysis, reporting, dashboard creation, and business operations support. Passionate about transforming data into actionable insights that drive business growth.

---

###  **Interactive Dashboards**

*  [Bright Coffee Shop Sales Dashboard (Version 1)](https://datastudio.google.com/reporting/04d746f9-48f5-4e39-8e22-5943b3b6085c)
*  [Bright Coffee Shop Sales Dashboard (Version 2)](https://datastudio.google.com/reporting/4251cf9c-2ead-4341-842c-f38975ad2f60)

---

###  **Key Objectives**

* Analyze revenue trends over time
* Evaluate product category performance
* Identify top-performing segments
* Support strategic decision-making with data insights
* Highlight opportunities to increase sales and customer engagement

---

###  **Tools & Skills Applied**

* SQL (Data Cleaning & Transformation)
* Excel / Google Sheets (Data Preparation & Pivot Tables)
* Looker Studio (Dashboard Design & Data Visualization)
* Data Analysis & Business Insight Generation

---

###  **Outcome**

This project delivers an interactive dashboard and actionable insights to help leadership:

* Improve revenue performance
* Optimize product strategy
* Make data-driven decisions

---

